...
def tag_modify(old_tests):
    ...
    for old_tag in tqdm(old_tests, unit= f' test element of vuln № {num+1}'):
...
def objects_modify(old_objects):
    ...
    for old_obj in tqdm(old_objects, unit= f' object element of vuln № {num+1}'):
        for obj in objects:
...
def states_modify(old_states):
    ...
    for old_state in tqdm(old_states, unit= f' state element of vuln № {num+1}'):
        for state in states:
...
def parser_starter(num):
    ...
    old_objects = soup.find_all('definition')[num].find('criteria').find_all('red-def:object') # + ('red-def:rpmverifyfile_test')
